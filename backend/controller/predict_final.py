...
class InclusionEvaluator(Evaluator):
    ...
    def evaluate(self, model, eva_samples, classes, classes_e, inferred_ancestors):
        ...
        for k, sample in enumerate(eva_samples):
            ...
            if rank_non > rank:
                csv_file_path = f'foodon_OWL2VECSTAR.csv'
    ...
    def run_random_forest(self, classes, classes_e, inferred_ancestors):
        ...
        MRR, hits1, hits5, hits10 = self.evaluate(model=rf_best, eva_samples=self.test_samples, 
                                                  classes=classes, classes_e=classes_e, inferred_ancestors=inferred_ancestors)
...
def predict_func(ontology_file, algorithm):
    # load config file
    ...
    if ontology_file.lower() == "foodon":
        ontology_file = "foodon-merged.train.owl"
    elif ontology_file.lower() == "helis":
        ontology_file = "helis_v1.00.train.owl"
    ontology_file_path = f'backend\ontologies\{ontology_file}' 
    ...
    #loading algorithm
    print("loading algo")
    model_path = f'backend\storage\model1\output'
    ...
    with open('backend\storage\inferred_ancestors.txt') as f:
    ...
    evaluator.run_random_forest(classes, classes_e, inferred_ancestors)
